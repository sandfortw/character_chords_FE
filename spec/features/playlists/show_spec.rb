    ...
    context 'When I visit my playlist show page' do
      before do 
        @playlist_json = {:links=>
          ...
          {"links"=>{"image"=>"www.roycohnlink.com"}, "data"=>{"type"=>"character", "theme_id"=>1, "attributes"=>{"character_id" => "7", "name"=>"Roy Cohn", "quiz"=>"lawyer", "alignment"=>"Lawful Evil"}}}, 

          {"links"=>{"image"=>"www.thurgoodmarshall.com"}, "data"=>{"type"=>"character", "theme_id"=>1, "attributes"=>{"theme_id" => "1", "character_id" => "1", "name"=>"Thurgood Marshall", "quiz"=>"lawyer", "alignment"=>"Lawful Good"}}}]
      ...
      describe "view on the page" do 
        before(:each) do 
          ...
          it "there is a location to remix the songs" do 
            ...
            expect(page).to have_select("character_select", :with_options => ["Michael Cohen", "Roy Cohn", "Thurgood Marshall"])
