...
RSpec.describe 'Playlist Show Page', type: :feature do
  ...
  describe 'As a logged in user' do
    context 'When I visit my playlist show page' do
      ...
      it 'I can open the playlist in Spotify', :vcr do
        playlist_json = {:links=>
        {:image=>
        ...
        facade = PlaylistFacade.new(params)
        allow(facade).to receive(:create_playlists).and_return(playlist_json)
        allow(PlaylistFacade).to receive(:new).and_return(facade)

        login_with_oauth

        VCR.use_cassette('spotify_playlist') do
          page.driver.post("/playlists")
        end  
    
        visit playlist_path(1)
